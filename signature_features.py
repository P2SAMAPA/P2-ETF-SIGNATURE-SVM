...
def signature_depth2(path):
    ...
    for i in range(2):
        for j in range(2):
            ...
            sig2[idx] = total
            idx += 1
    return np.concatenate([sig1, sig2])      # (6,)

def signature_depth3(path):
    """Depth 3 signature (14 features)."""
    if path is None or len(path) < 2:
        return None
    inc = np.diff(path, axis=0)
    L = inc.shape[0]
...
def rolling_signatures(price_series, window, depth=2):
    """
    For a single ETF price series, compute signatures for each rolling window.
    Returns (signatures, labels).
    """
    signatures = []
    labels = []
